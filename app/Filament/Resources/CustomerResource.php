...
class CustomerResource extends Resource
{
            ...
            ->schema([
                ...
                Forms\Components\Section::make(__('Product Type'))
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('product_price_usd')
                ...
                // ── نوع التسديد والجدول ──
                Forms\Components\Section::make('جدول التسديد')
                    ->icon('heroicon-o-calendar-days')
                    ->schema([
                        Forms\Components\Select::make('payment_type')
                            ->label('نوع التسديد')
                            ->options(collect(PaymentType::cases())->mapWithKeys(fn ($t) => [$t->value => $t->label()]))
                ...
                Infolists\Components\Section::make(__('Product Type'))
                    ...
                    ->schema([
                        ...
                        Infolists\Components\TextEntry::make('product_sale_total')
                            ->label(__('Sale Total'))
                            ->formatStateUsing(fn (int $state): string => Number::iqd($state)),
                        Infolists\Components\TextEntry::make('product_price_usd')
                            ->label('السعر بالدولار')
                            ->formatStateUsing(fn (?int $state): string => $state ? '$' . number_format($state) : '-'),
                    ])->columns(4),

                Infolists\Components\Section::make('جدول التسديد')
                    ->icon('heroicon-o-calendar-days')
                    ->schema([
                        Infolists\Components\TextEntry::make('payment_type')
                            ...
                            ->color(fn (PaymentType $state): string => $state === PaymentType::LumpSum ? 'warning' : 'primary'),
                        Infolists\Components\TextEntry::make('delivery_date')
                            ->label(__('Delivery Date'))
                            ->date('Y/m/d'),
                        Infolists\Components\TextEntry::make('payment_due_date')
                            ->label('تاريخ التسديد')
                            ->date('Y/m/d')
                            ->placeholder('-'),
    ...
    {
        $total = (int) str_replace(['.', ','], '', (string) $get('product_sale_total'));
