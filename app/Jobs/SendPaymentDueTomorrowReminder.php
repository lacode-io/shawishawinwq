...
class SendPaymentDueTomorrowReminder implements ShouldQueue
{
    ...
    public function handle(WhatsAppManager $whatsapp): void
    {
        ...
        if ($this->overridePhone === null
            && ! $whatsapp->canSend($to, 'payment_due_tomorrow')) {
            return;
        }

        $whatsapp->send(
            to: $to,
            message: MessageTemplates::paymentDueTomorrow($this->customer),
            messageType: 'payment_due_tomorrow',
