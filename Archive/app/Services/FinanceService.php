...
class FinanceService
{
    ...
    public function cashCapital(): int
    {
        return Cache::remember('finance.cash_capital', self::CACHE_TTL, function () {
            ...
            return $manualCapital + $totalPaymentsIn - $totalExpenses - $totalInvestorPayouts;
        });
    }
    ...
    /**
     * المستحقات للمستثمرين = مبلغ الاستثمار + المبلغ المدفوع فعلياً لكل مستثمر فعال
     */
    public function investorsDueTotal(): int
    {
        return Cache::remember('finance.investors_due', self::CACHE_TTL, function () {
            ...
            return (int) $investors->sum(fn (Investor $i) => $i->amount_invested + $i->total_paid_out);
        });
    }
