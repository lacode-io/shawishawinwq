...
class InvestorResource extends Resource
{
                ...
                Forms\Components\Section::make(__('Amount Invested'))
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('monthly_target_amount')
                            ...
                            ->numeric()
                            ->suffix(__('IQD'))
                            ->helperText('يتم حسابه تلقائياً (المبلغ + الربح) ÷ الأشهر'),
                    ])->columns(2),
    ...
    {
        $amount = (int) $get('amount_invested');
        ...
        if ($amount > 0 && $percent > 0 && $months > 0) {
            ...
            $totalDue = $amount + $totalProfit;
            $set('monthly_target_amount', (int) ceil($totalDue / $months));
        }
    }
}
