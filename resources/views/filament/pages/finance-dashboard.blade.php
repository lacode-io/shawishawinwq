<x-filament-panels::page>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {{-- الربح الشهري --}}
        <div wire:key="profit-{{ $this->profitMonth }}-{{ $this->profitYear }}">
            <x-filament::section>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-xs text-gray-400 dark:text-gray-500">الإجمالي − رأس المال | debug: month={{ $stats['profit_month_val'] }} year={{ $stats['profit_year_val'] }}</div>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <select wire:model.live="profitYear"
                            class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300">
                            @foreach(range(now()->year + 1, 2024, -1) as $y)
                                <option value="{{ $y }}">{{ $y }}</option>
                            @endforeach
                        </select>
                    </div>
        ...
        <x-filament::section>
            <div class="flex items-center gap-3">
                <div class="rounded-lg {{ $stats['annual_profit'] >= 0 ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20' }} p-2">
                    <x-heroicon-o-chart-bar class="h-6 w-6 {{ $stats['annual_profit'] >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}" />
                </div>
                <div>
